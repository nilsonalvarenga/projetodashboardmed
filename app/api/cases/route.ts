...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const row = { ...caso, titulo: (caso.titulo || caso.queixa_principal || 'Caso clínico').slice(0, 80) };
    const { data, error } = await supabaseAdmin().from('clinical_cases').insert(row).select('id').single();
    if (error) throw new Error(error.message);
    await supabaseAdmin().from('audit_logs').insert({ action: 'case:create', entity: 'case', entity_id: data.id });
